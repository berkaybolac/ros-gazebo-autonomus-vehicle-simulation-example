...
def mybot_control(data):
    if data[0:18].min() > 0.6 and data[0:18].min() <= 3.0 : # Robotun onunden 80-100 derece arasına bakıyoruz.
        speed = -0.5
        if data[10:14].min()  -  data[5:9].min() <= 1.0: # Robotun 100-140 ile 50-90 derece arasındaki farka gore robotu yonlendiriyoruz.
            print "sag"
            speed = -1.0
            angle = 5.0 # Posif deger sol
        else :
            print "sol"
            speed = -1.0    
            angle = -5.0 # Negatif deger sag
    
    elif data[0:18].min() <= 1: #robotun engele 1.0 den daha az mesafede ise robotu durdur.
        print "dur"
        speed = 0.0
        angle = 0.0
